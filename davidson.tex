...
\KwData{$D$ the array of determinants present in $\ket \Psi$, sorted by alpha-major order}
\KwData{$A$ the array so that $A[n]$ is the index of the first occurence of the $n^{th}$ unique alpha bitstring in $D$, and $A[N_a +1] = \Ndet+1$ for algorithmic convenience}
