...
which aims at extracting the first few $\Nstate$ lowest eigenstates of a large
matrix. This algorithm reduces the cost of the computation
to $\order{\Nstate \Ndet^2}$, and of the storage to $\order{\Nstate \Ndet}$.
...
\For{$b2 \gets b1+1, A(a+1)-1$}{
	\If{$\text{EXC\_DEGREE}(D[b1]_\beta, D[b2]_\beta) \leq 2$}{
		$\ket {D[b1]}$ connected to $\ket {D[b2]}$ by single or double $\beta$ excitation.	
	}
}
}
}
\tcc{Single and double $\alpha$ excitations are found by the same algorithm after flipping spins}
\end{algorithm}
...
as they scale as~$\order{N_a  N_b^2} = \order{N_a^3}$. Indeed, the \texttt{cycle} instruction at line~4 makes the
