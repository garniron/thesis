...
\begin{algorithm}
	...
	\For{$t \gets 2,N_{teeth}$}{
		$r = u_0 + W_T \times (t-1)$ \;
		$n_0(t) = FIND\_SAMPLE(r, P)$ \;
	}
	\tcc{for convenience}
	$n_0(\Nteeth+1) = \Ngen$ \;
\end{algorithm}
...
\begin{algorithm}
	...
	\For{$t \gets 1,\Nteeth$}{
		$\text{tooth\_width} = P_{n_0(t+1)} - P_{n_0(t)}$ \;
		\For{$I\gets n_0(t)+1, n_0(t+1)$}{
		$w_{I} = p(e_I) \times \frac{W_T}{\text{tooth\_width}}$ \;
		}
	}
...
\alert{TODO : REVOIR APRES TASK MONTE CARLO}
...
\begin{algorithm}
	...
	\tcc{\alert{For convenience, makes sure the last task matches the last comb, rather than a tooth filling.}}
	\If{$\Ngen > 1$}{
		$R[N_j-1] \gets 0$ \;
		$R[N_j] \gets N_c$ \;
	}
...
\begin{algorithm}
	...
	\label{alg:ADD_COMB}
		\KwData{$N_c,u[N_c],F,d,N_j,Q$ as in the scope of the calling function}
...
\begin{algorithm}
	...
	\KwData{$F$,$d$,$N_j$,$Q$ as in the scope of the calling function}
...
\begin{algorithm}
	...
	\While{$n \leq \Ngen$}{
		\uIf{$f[Q[n]] = 0$}{
			...
			\tcc{short-circuit boolean evaluation required to prevent out of bound access to $n_0$}
...
\begin{algorithm}
	...
	$x \gets 0$ \;
	\For{$t \gets N_{teeth} \text{ down to } 1$}{
