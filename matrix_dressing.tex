...
\item
In the case of PT2 (at least in its Epstein-Nesbet version), each connection found only requires an increment of some elements of $P(G_{pq})$. At no point 2 connections need to be known at the same time. This is different for methods implemented with matrix dressing. It is possible that one needs to know the detail of which internal determinants an $\ket \alpha$ connects to, in order to be able to compute $\delta_\alpha$.
\end{itemize}
...
\begin{algorithm}
	\caption{Compute $\tilde \epsilon$, $\dot t$ and $\dot n_0$}
	\label{COMPUTE_EPSILON}
		\KwData{$J$, $R^{-1}$, $n_0$, $N_{teeth}$}
		\KwResult{ $\tilde \epsilon$, $\dot t$ and $\dot n_0$}
		...
		\For{$m=N_{cp},2,-1$}{
			$\tilde \epsilon^m_* \gets \tilde \gamma^m_* - \tilde \gamma^{m-1}_*$ \;
		}
\end{algorithm}
...
\begin{algorithm}
	...
	\While{$\cpdone > \cpsent \vee m < N_{cp}+1$ }{
		...
		--END OMP CRITICAL-- \;
		\If{$\willsend \neq 0$}{
			send $\kappa^{\willsend}$ (shown in algorithm \ref{alg:build_checkpoints})
		}
		\If{$m < N_{cp}+1$}{
...
\begin{algorithm}
	\caption{Perform task $(I,s)$, that is, update \alert{`node-local''} partial values of $ \kappa^{D,t}$ and $ \kappa^{S,m}$.}
	\label{alg:update_checkpoints}
	\KwData{global shared scope (at node level): $\kappa^{D,t}$ and $\kappa^{S,m}$ initialized with $0$, $f$ an array size $\Ngen$ initialized with $0$. The fragmentation count}
	...
	compute $\kappa_{I,s}$ (\alert{fragment $s$ of $\kappa_{I}$})\;
	\tcc{lock global arrays before update}
	$\kappa^{D,t} \gets \kappa^{D,t} + \kappa_{I,s}$ \;
	\For{$p=m,N_{cp}$}{
		\If{$\tilde \epsilon^p_I \neq 0$}{
			$\kappa^{S,p} \gets \kappa^{S,p} + \tilde \epsilon^{p}_I \kappa_{I,s}$ \;
		}
	}
	$x \gets {\bf c} \cdot \kappa_{I,s}$ \;
	--ATOMIC-- \;
	$e_I \gets e_I + x$ \;
	$f[I] \gets f[I] + 1$ \;
	--END ATOMIC-- \;
...
\begin{algorithm}
	\caption{Build $ \kappa^m$ and send it to master node.}
	\label{alg:build_checkpoints}
	\tcc{compute partial value of $\kappa^m$ with partial values of $\kappa^{D,t}$ and $\kappa^{S,m}$}
	...
	\For{$p=m,1,-1$}{
		$\kappa^m \gets \kappa^m + \kappa^{S,p}$
	}
	$\kappa^m \gets \frac{\kappa^m}{M_m}$ \; 
	\For{$t=\dot t_m-1,0,-1$}{
		$\kappa^m \gets \kappa^m + \kappa^{D,t}$ \;
	}
	%$\mathcal{I}$ the set of $I \in \mathcal{P}_m$ with $f[I] > 0$ \;
	send ($m$, $\kappa^m$, $e_{I \in \mathcal{P}_m}$, $\dot f$) \;
\end{algorithm}
...
\begin{algorithm}
	...
	\While{}{
		...
		\Else{
			...
			retrieve $(\breve l,  \breve {\kappa}^l$, $ \breve {e}_{I \in \mathcal{P}_l}, \breve f)$  \;
			increment $e_{I \in \mathcal{P}_l}$ with $\breve e_{I \in \mathcal{P}_l}$\;
			increment $\kappa^l$ with $\breve  \kappa^l$ \;
