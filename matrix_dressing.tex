...
\begin{algorithm}
	...
	Compute $\deltabold_{I,s}$ (\alert{fragment $s$ of $\deltabold_{I}$})\;
...
\begin{algorithm}
	...
	$\deltabold^m \gets 0$ \;
	\tcc{Reverse loop for numerical precision \alert{POURQUOI?}}
	\For{$p\gets m \text{ down to }1$}{
		$\deltabold^m \gets \deltabold^m + \deltabold^{S,p}$
	}
	$\deltabold^m \gets \frac{\deltabold^m}{M_m}$ \; 
	\For{$t\gets \dot t_m-1,0,-1$}{
