...
The presented algorithm generates a wavefunction which is expressed on a truncated space of Slater determinants. Determinants are not necessarily eigenfunctions of the $\widehat S^2$ operator, so the eigenfunctions of the Hamiltonian are not guaranteed to be also eigenfunctions of $\widehat S^2$.

A conventional solution for truncated CI methods is to work in the basis of \emph{Configuration State Functions} (CSFs). These are linear combinations of determinants which are eigenfunctions of $\widehat S^2$, and with the desired eigenvalue. Diagonalizing $\widehat H$ in this basis ensures that the solution is spin pure.
...
\end{align}
which is an estimated cost of task $i$. In practice $F_i = 1$ for the majority.
...
\item
Although not directly related to the CIPSI algorithm, the computation of said phase factors has been made much cheaper thanks to the use of phase masks (see chapter \ref{chap:PHASEMASK})
\item
