...
\begin{algorithm}
	\caption{Simplified CIPSI selection}
	\label{alg:selection}
	\KwData{ $\Psi$, $\{D_g\}$}
	...
	\ForAll{$(p,q) \; ; \; a_p a_q D_g \neq 0$}{
		...
		\For{$p \gets 1,N_{det}$}{
			...
			\If{$\POPCNT{c} = 2$}{
				$e \gets \mathtt{LIST\_FROM\_BITSTRING}(c)$\; 
				$B_{e_1, e_2} \gets \mathtt{FALSE}$\; 			  
			}			
			\tcc{\alert{see table XXX for the loops}}
			\uIf{$p < g$}{
