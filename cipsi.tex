...
Essentially, determinants in a batch are defined by their difference to $G_{pq}$. Therefore, comparing $G_{pq}$ to\ selctor determinant, allows to systematically determine which $\kalpha$ of the batch it will connect to, and by what excitation. Additional filtering mechanisms are set up to avoid consideri\nselctors that do not interact with the current batch. Those will be made explicit later on. Comparing figures \ref{fig:old_cipsi} and \ref{fig:new_cipsi} hints the differences between the former an newer algorithm. Note that because generators are a subset \oselctors, a particular $\kalpha$ generated from $\ket {D_g}$ must be checked for connection to a\lselctors either as generators or \aselctors.

\begin{itemize}
\item
$\ket {D_{I \in [1,g-1]}}$ as generators to check if $\kalpha$ has been previously generated
\item
$\ket {D_{I \in [g,\Nsel]}}$ as selectors to compute $\Hij{\Psi}{\alpha}$
\end{itemize}
...
\If{$a \leq 2$}{
	find $S_\beta \in \{U_\beta\}_{G_\alpha}$ so that $\text{EXC\_DEG}(S_\beta, G_\beta) + a \leq 4$  \; 
}
}
\For{$S_\beta \in \{U_\beta \}$}{
$b \gets \text{EXC\_DEGREE}(S_\beta, G_\beta)$ \;
\If{$b \leq 1$}{
	find $S_\alpha \in \{U_\alpha\}_{G_\beta}$ so that $\text{EXC\_DEG}(S_\alpha, G_\alpha) + b \leq 4$  \; 
}
}
\end{algorithm}
...
\begin{algorithm}
	...
	\tcc{A logical task is computation of $e[i]$ the the sum of $\epsilon(\alpha)$ over all unique $\kalpha$ generated from generator $\ket {G_i}$}
	chose $F_i$ \;
	\For{$i=1,\Ngen$}{
	\For{$s=0,F_i-1$}{
