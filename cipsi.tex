...
\item
If $J \geq I$, increment all untagged $P_{rs}(G_{pq})$ matrix elements by $\Delta P_{rs}(G_{pq}) = c_J \Hij{S}{G^{rs}_{pq}}$. \alert{JE NE COMPRENDS PAS DU TOUT CETTE PHRASE:} Note that $\overrightarrow{T}$ so that $\ket S=\overrightarrow{T} \Gpqrs$ can be determined at the same time as the $(r,s)$ pair.
\item
...
\begin{algorithm}
	...
	\label{alg:unblock_single}
		...
		\If{($q$ is of spin $\alpha$) and ($p=M_\beta$)}{
				$B_{*,M_\beta} \gets \mathtt{TRUE}$ \;	
				$B_{M_\beta,*} \gets \mathtt{TRUE}$ \;		
			}
			
			\If{($p$ is of spin $\beta$) and ($q=M_\alpha$)}{
				$B_{*,M_\alpha} \gets \mathtt{TRUE}$ \;	
				$B_{M_\alpha,*} \gets \mathtt{TRUE}$ \;		
			}
\end{algorithm}
