...
\begin{algorithm}
	...
	\tcc{This takes place after $\tilde{\mathcal{C}}_{rs}$ and $B_{rs}$ for a given batch have been fully computed}
		...
		$T$ an array size $\Nsel$ initialized to $\TRUE$ \;
		$L$ an array of determinants size $\Nsel$ \;
        $i_1 = |\interC{00}|$ \;   
        $L[1 \ldots i_1] \gets \interC{00}[1 \ldots {i_1}]$ \;
        
		\tcc {$B_{r0} = \FALSE$ if column $r$ is entirely tagged}		
		\ForAll{$r ; B_{r0}$}{
		  $i_2 = i_1 + |\interC{r0}|$ \;
...
\begin{equation}
c_r \Phi(R _r \rightarrow \alpha) \frac{t_{R _r \rightarrow D_i}}{\Phi(R _r \rightarrow D_i)} \frac{t_{D_i \rightarrow \alpha}}{\Phi(D_i \rightarrow \alpha)} =c_r t_{R_r \rightarrow D_i} t_{D_i \rightarrow \alpha}
\end{equation}
...
\begin{equation}
c_r t_{R _r \rightarrow D_i} t_{R _r \rightarrow D_j} \Phi(R _r \rightarrow D_j)\Phi(D_i \rightarrow \alpha)
\end{equation}
